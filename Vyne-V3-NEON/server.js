...
const httpServer = http.createServer((req, res) => {
  // Privacy/security headers. The signaling server never persists call media.
  res.setHeader("X-Content-Type-Options", "nosniff");
  res.setHeader("Referrer-Policy", "no-referrer");
  res.setHeader("X-Frame-Options", "DENY");
  res.setHeader("Permissions-Policy", "camera=(self), microphone=(self), display-capture=(self)");
  res.setHeader("Cache-Control", "no-store");
  // Keep CORS permissive for a separately hosted frontend; tighten this to your
  // exact frontend origin in production when you do not need cross-origin access.
